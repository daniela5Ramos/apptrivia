...
const getTriviaById = async (id) => {
    try {
      const response = await axios.get(`http://localhost:3000/api/trivias/${id}`); // Usar comillas invertidas para interpolación
      return response.data;
    } catch (error) {
  ...
  const getTriviasCompletadas = async (idUsuario) => {
    try {
      const response = await axios.get(
        `http://localhost:3000/api/resultados/usuario/${idUsuario}`
      );
